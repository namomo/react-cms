...
### 1. 확장 가능한 디렉토리 구조
유지보수와 확장을 고려하여 역할과 도메인별로 명확히 분리된 구조를 가집니다. `src/` 하위에 핵심 관심사들이 나뉘어져 있습니다.
- **kebab-case** 파일/폴더명 및 **PascalCase** 컴포넌트명 규칙을 엄격히 준수합니다.
- `api/`: API 클라이언트(`api-client.js`)와 더미 데이터(`dummy/`) 모듈
- `components/`: 재사용 가능한 전역 컴포넌트 (`common/`, `layout/`)
- `config/`: 환경 변수 및 공통 설정
- `hooks/`: 커스텀 훅
- `layouts/`: 페이지의 공통 껍데기를 담당하는 레이아웃 (`main-layout`, `auth-layout`)
- `pages/`: 도메인 단위 페이지 화면 (`auth`, `dashboard`, `settings`, `content` 등). 특정 페이지에 필요한 자식 컴포넌트나 로컬 상태는 해당 디렉토리 내에 응집시킵니다.
- `routes/`: 대메뉴 단위로 라우팅 객체를 분리(예: `settings-router.jsx`)하여 최상위 `app-router.jsx`에서 Spread Operator(`...`)로 병합합니다. 이를 통해 라우터 파일이 비대해지는 것을 방지합니다. 레이아웃 기반으로 인증이 필요한 경로는 `<MainLayout />`, 로그인은 `<AuthLayout />`으로 나뉩니다.
- `services/`: 비즈니스/API 연동 로직
- `stores/`: Zustand 전역 상태 저장소 (`auth-store.js`, `ui-store.js`)

### 2. 진입점 로직 (`main.jsx` & `App.jsx`)
- **초기화 대기**: `App.jsx`에서 `useAuthStore`의 `verifyAuth()`를 `useEffect`로 실행하며 검증할 때까지 로딩 화면 레이아웃을 띄웁니다(`isInitialized` 플래그 활용). 이를 통해 토큰 확인 과정에서의 깜빡임(Flickering)과 무단 라우트 렌더링을 막았습니다.

### 3. Service Layer & 의존성 역전(IoC) 컴포넌트 연동 패턴
...
### 4. 상태 관리 및 세션 유지 (Token Verification)
- **Zustand 스토어 (`src/stores/auth-store.js`, `ui-store.js`)**: 로그인한 사용자 정보, 인증 토큰 등 렌더링에 관여하는 동적 전역 상태를 관리합니다. 특히 `verifyAuth` 함수를 통해 새로고침이나 첫 진입 시 `localStorage`의 토큰을 읽어와 로그인 상태를 자동으로 복원하거나 유효성을 검사하는 처리가 깔끔하게 작성되어 있습니다. `ui-store.js`는 UI 관련 상태 변수(다크모드, 툴바 등)를 제어합니다.
- **새로고침 세션 방어 (Flicker Prevention)**: 브라우저가 새로고침되어 앱이 재구동될 때, 토큰 검증이 실패(만료 등)하면 스토어를 초기화하여 로그아웃 처리하고 로그인 뷰로 이동시킵니다.
